...
param (
    ...
    [ValidateScript({Test-Path -Path $_})][string]$ESXi65aOfflineBundle,
    ...
    [parameter(ValueFromPipelineByPropertyName=$true)][string]$PrivateVXLANVMNetwork = "dv-private-network",
    ## VDS / VXLAN Configurations -- name to use for when creating new VDSwitch in new virtual datacenter for use by NSX
    [parameter(ValueFromPipelineByPropertyName=$true)][string]$VDSName = "VDS-6.5",
    ## VDS / VXLAN Configurations -- name to use for when creating new VXLAN VDPortgroup on new VDSwitch for NSX
    [parameter(ValueFromPipelineByPropertyName=$true)][string]$VXLANDVPortgroup = "VXLAN",
    ...
    [parameter(ValueFromPipelineByPropertyName=$true)][System.Net.IPAddress]$VXLANSubnet = "172.16.66.0",
    ...
    [parameter(ValueFromPipelineByPropertyName=$true)][System.Net.IPAddress]$VXLANNetmask = "255.255.255.0",
...
if($confirmDeployment -eq 1) {
    ...
    Write-Host -ForegroundColor White "$NestedESXivMemGB GB"
    Write-Host -NoNewline -ForegroundColor Green "Caching VMDK: "
    Write-Host -ForegroundColor White "$NestedESXiCachingvDiskGB GB"
    Write-Host -NoNewline -ForegroundColor Green "Capacity VMDK: "
    Write-Host -ForegroundColor White "$NestedESXiCapacityvDiskGB GB"
    ...
    if($bDeployNSX) {
        ...
        Write-Host -ForegroundColor White $NSXvMemGB
        Write-Host -NoNewline -ForegroundColor Green "Hostname: "
    ...
    $esxiTotalMemory = $NestedESXiHostnameToIPs.count * [int]$NestedESXivMemGB
    $esxiTotalStorage = ($NestedESXiHostnameToIPs.count * [int]$NestedESXiCachingvDiskGB) + ($NestedESXiHostnameToIPs.count * [int]$NestedESXiCapacityvDiskGB)
...
if($setupNewVC -eq 1) {
    ...
    if($bDeployNSX -and $setupVXLAN -eq 1) {
        ...
        foreach ($vmhost in $vmhosts) {
            ...
            $vxlanVmkIP = ($VXLANSubnet.Split(".")[0..2],$lastNetworkOcet | Foreach-Object {$_}) -join "."
